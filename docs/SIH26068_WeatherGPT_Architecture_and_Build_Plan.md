...
- **Deployment — DONE.** Backend live at `https://weathergpt-sih2026.onrender.com` (with `WEATHERAPI_KEY` set) and frontend static site live at `https://weathergpt-sih2026-frontend.onrender.com`, wired via `VITE_API_BASE_URL` with CORS opened through `FRONTEND_ORIGINS`. Config is reproducible in `render.yaml` + `docs/DEPLOYMENT.md`. This **closes the Sprint 1 checkpoint**: real, structured, gracefully-degrading weather data (WeatherAPI → Open-Meteo → historical) for any Indian location, deployed end-to-end, with 61 backend + 19 frontend tests green in CI.
