...
| Travel Planner tab | route/checkpoint UI shipped (mock data) | `POST /trip-plan` backend: **Geoapify** (geocoding + routing + Places for facilities) + Open-Meteo weather at each ETA. Needs a `GEOAPIFY_API_KEY` secret (the project's first keyed external API) + free-tier rate handling. Section 3.10. |
