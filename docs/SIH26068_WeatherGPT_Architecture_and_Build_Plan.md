...
- **Deployment** — backend is **live** on Render's free tier (`https://weathergpt-sih2026.onrender.com`), with `WEATHERAPI_KEY` set. Config is captured for reproducibility in `render.yaml` + `docs/DEPLOYMENT.md`. **Remaining Sprint-1 step:** deploy the frontend as a Render static site (`docs/DEPLOYMENT.md` §2) and point the backend's `FRONTEND_ORIGINS` at it (§3).
